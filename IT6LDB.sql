...
--
-- Host: localhost
-- Generation Time: Jan 20, 2024 at 10:49 AM
...
(153, 'Christopher', 'Olsen', 'COLSEN', '011.44.1344.498718', '1998-03-30', 'SA_REP', 8000, '0.20', 145, 80),
(154, 'Nanette', 'Cambrault', 'NCAMBRAU', '011.44.1344.987668', '1998-12-09', 'SA_REP', 7500, '0.20', 145, 80),
...
(500, NULL, 'sample', 'asa', NULL, '2010-04-24', 'AD_VP', 50000, NULL, NULL, 50);

-- --------------------------------------------------------

--
-- Stand-in structure for view `emp_job_view`
-- (See below for the actual view)
--
CREATE TABLE `emp_job_view` (
...
('CCE103', 'STUDENT', 0, '10000'),
('FI_ACCOUNT', 'Accountant', 4200, '9000'),
('FI_MGR', 'Finance Manager', 8200, '16000'),
('HR_REP', 'Human Resources Representative', 4000, '9000'),
('IT6l', 'DATABASE', 10000, '100000'),
...
--
DELIMITER $$
CREATE TRIGGER `checkSalary` BEFORE INSERT ON `jobs` FOR EACH ROW BEGIN 
IF NEW.min_salary < 1000 THEN 
SET NEW.min_salary = 0;
END IF;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `getTotalJobs` AFTER INSERT ON `jobs` FOR EACH ROW BEGIN 
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `saveLog` AFTER INSERT ON `jobs` FOR EACH ROW BEGIN 

DECLARE DESCRIPTION_DATA TEXT;
SET DESCRIPTION_DATA = CONCAT('New inserted job : ', NEW.job_id);

INSERT INTO logtbl(description,date_added) VALUES (DESCRIPTION_DATA, NOW());
...
INSERT INTO `logtbl` (`id`, `description`, `date_added`) VALUES
(1, 'New inserted job : ', '2023-09-05'),
(2, 'New inserted job : CCE101', '2023-09-05'),
(3, 'New inserted job : CCE103', '2023-09-05'),
(4, 'New inserted job : IT6l', '2023-09-05');
...
INSERT INTO `user_table` (`id`, `username`, `password`, `name`) VALUES
(2, 'samsepiol', 'samsepiol', 'Sam Sepiol'),
(3, 'cornytoe', 'cornytoepass', 'Corny Toe'),
(4, 'stud_dante', 'iamstudent', 'Stud D. Ante'),
(5, 'johnnydepp', 'jphnyjphnyyespapa', 'Johnny Depp'),
(6, 'maryshaw', 'maryhawpassword', 'Mary Shaw'),
(7, 'carnie_norte', 'thisisapassword', 'Carnie Norte'),
(14, 'cliff_sama', 'cliff_sama', 'Cliff Sama'),
(16, 'rayeban_2023', 'rayeban_2023', 'Raye D. Ban'),
(22, 'admin123', 'admin123', 'admin'),
(27, 'qwerty123', 'qwerty123', 'Sam Sepiol');

-- --------------------------------------------------------

--
-- Structure for view `emp_job_view`
--
DROP TABLE IF EXISTS `emp_job_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `it6ldb`.`emp_job_view`  AS SELECT `it6ldb`.`employees`.`employee_id` AS `employee_id`, `it6ldb`.`employees`.`last_name` AS `last_name`, `it6ldb`.`employees`.`first_name` AS `first_name`, `it6ldb`.`jobs`.`job_title` AS `job_title` FROM (`it6ldb`.`employees` join `it6ldb`.`jobs` on(`it6ldb`.`employees`.`job_id` = `it6ldb`.`jobs`.`job_id`))  ;
